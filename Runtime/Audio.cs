    ...
    public class Audio
    {
        ...
        public Transform SourceTransform
        {
            ...
            set
            {
                if(value == null)
                {
                    sourceTransform = SoundManager.Instance.transform;
                }
                else
                {
        ...
        public Audio(AudioType audioType, AudioClip clip, bool loop, bool persist, float volume, float fadeInValue, float fadeOutValue, Transform sourceTransform)
        {
            ...
            Pitch = 1;
            StereoPan = 0;
            if (sourceTransform != null && sourceTransform != SoundManager.Instance.transform)
            {
                SpatialBlend = 1;
            }
            ReverbZoneMix = 1;
        ...
        public void Update()
        {
            ...
            switch (Type)
            {
                case AudioType.Music:
                    {
                        AudioSource.volume = Volume * SoundManager.Instance.MusicVolume * SoundManager.Instance.GlobalVolume;
                        break;
                    }
                case AudioType.GameplaySound:
                    {
                        AudioSource.volume = Volume * SoundManager.Instance.GameplaySoundsVolume * SoundManager.Instance.GlobalVolume;
                        break;
                    }
                case AudioType.UISound:
                    {
                        AudioSource.volume = Volume * SoundManager.Instance.UISoundsVolume * SoundManager.Instance.GlobalVolume;
                        break;
                    }
            }
